<script>
  ...
  const getSelects = async() => {
    ...
    try {
      const r = await fetch(
        `${__myapp.env.API_URL}/selected?date=${date}`
      )
      if (r.status === 200) {
  ...
  const getComment = async(target) => {
    const r = await fetch(
      `${__myapp.env.API_URL}/comments?target=${target}`
    )
    if (r.status === 200) {
      const j = await r.json()
      return j
    ...
    <div slot="foreground">
      ...
      <div style="padding: 0">
        ...
        {#each ['蔡', '勋', '程', '杜', '姜', '何'] as u}
          <FormField>
            <Radio bind:group={self} value={u} on:change={() => localStorage.setItem('user', u)}/>
      ...
      <DataTable table$aria-label="People list">
        ...
        <Body>
          ...
          <Row>
            ...
            <Cell>
              <Button variant="outlined" color={selected[item.id] === undefined ? "" : "secondary"} on:click={() => handleSelect(item.id)}>
                {#if selected[item.id]}
                  取消
                {:else}
                  点菜
            ...
            <Cell>
              <IconButton class="material-icons" on:click={() => updateComment(index, 1)}>thumb_up</IconButton>
              <IconButton class="material-icons" on:click={() => updateComment(index, -1)}>thumb_down</IconButton>
            </Cell>
          </Row>
          {/each}
        </Body>
